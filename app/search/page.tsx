...
export default function UserPortal() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.success) {
        setToken(pin.trim())
        const client = data.client === 'Nikunj Goyal' ? 'Demo User' : (data.client || 'Demo User')
          ...
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-sm text-gray-700">
                Welcome, <span className="font-semibold">{clientName === 'Nikunj Goyal' ? 'Demo User' : (clientName || 'Demo User')}</span>.
              </span>
